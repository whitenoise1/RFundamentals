...
Point-in-time fundamental database for S&P 500 constituents. Builds a daily-frequency matrix of 74 indicators for ~500 stocks from free public data (SEC EDGAR + Yahoo Finance). Designed as input to quantitative factor models.

## What it does

- Fetches XBRL financial statements from SEC EDGAR for all current and historical S&P 500 members
- Computes 74 fundamental indicators per ticker per trading day (valuation, profitability, growth, leverage, efficiency, cash flow quality, shareholder return, size, plus academic anomaly factors)
...
```r
# data.table: date, price, pe_trailing, roe, ... (74 indicator columns)
...
```r
# list with $raw and $zscored data.tables (~500 tickers x 74 indicators)
...
| `get_indicator_names()` | character vector | 74 indicator names |
| `build_timeseries()` | (side effect) | Historical build, all tickers |
| `update_all_daily()` | (side effect) | Daily incremental update |

## Indicators

74 indicators across 11 categories. Full formulas, XBRL tags, academic references, and interpretation in [`docs/INDICATORS.md`](docs/INDICATORS.md).
...
12 indicators are price-sensitive (update daily with market close). 62 are fundamental-only (update when a new SEC filing appears).
...
R/
  ...
  indicator_compute.R      Pure computation, 74 indicators
